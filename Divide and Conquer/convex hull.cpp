...
int orientation(pair<int, int> p, pair<int, int> q, pair<int, int> r) {
    int val = (q.second - p.second) * (r.first - q.first) - (q.first - p.first) * (r.second - q.second);  // cross product
    if (val == 0) return 0;
    return (val > 0) ? 1 : -1;
}
...
vector<pair<int, int>> merger(vector<pair<int, int>> a, vector<pair<int, int>> b) {
    ...
    int ia = 0, ib = 0;
    // finding the rightmost point of a
    for(int i=1; i<n1; i++) {
        if (a[i].first > a[ia].first) ia = i;
    }
    // finding the leftmost point of b
    for(int i=1; i<n2; i++) {
    ...
    while(!done) {
        ...
        while(orientation(b[indb], a[inda], a[(inda+1) % n1]) >= 0) {  // a[(inda+1) % n1] is the next point of a[inda]
            // if a[inda] is on the right of line from b[indb] to a[(inda+1) % n1]
            inda = (inda + 1) % n1;
        }
        while(orientation(a[inda], b[indb], b[(n2+indb-1) % n2]) <= 0) {
            // if b[indb] is on the right of line from a[inda] to b[(n2+indb-1) % n2]
            indb = (n2+indb-1)%n2;
            done = 0;
        }
    }
